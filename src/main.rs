...
async fn create_new_blog(name: &str, body: &str, author: &str) -> Result<()> {
        ...
        if let Some(event) = evt {
            match event {
                ...
                EventType::Input(line) => match line.as_str() {
                    ...
                    cmd if cmd.starts_with("ls r") => handle_list_blogs(cmd, &mut swarm).await,
                    cmd if cmd.starts_with("create r") => handle_create_blog(cmd).await,
                    cmd if cmd.starts_with("publish r") => handle_publish_blog(cmd).await,
...
async fn handle_create_blog(cmd: &str) {
    if let Some(rest) = cmd.strip_prefix("create r") {
        ...
        if elements.len() < 3 {
            info!("too few arguments - Format: name|body|author");
        } else {
            let name = elements.get(0).expect("name is there");
            let body = elements.get(1).expect("body is there");
            let author = elements.get(2).expect("author is there");
            if let Err(e) = create_new_blog(name, body, author).await {
